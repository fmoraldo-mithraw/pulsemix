...
fun ManualTagDialog(
    ...
    // Plusieurs albums correspondent : choix de la pochette par l'image.
    // Composé après le dialogue principal, il s'affiche par-dessus.
    // Seuls les choix demandés pour CE morceau sont montrés : une
    // recherche d'un autre morceau, aboutie tardivement, est ignorée.
    val coverChoicesState by vm.manualCoverChoices.collectAsStateWithLifecycle()
    val coverChoices = coverChoicesState
        ?.takeIf { it.first == track.uri }?.second.orEmpty()
    if (coverChoices.isNotEmpty()) {
        AlertDialog(
            onDismissRequest = { vm.dismissCoverChoices() },
            title = { Text("Choisir la jaquette") },
            text = {
                Column {
                    Text(
                        "Plusieurs albums correspondent — touche la bonne " +
                            "pochette :",
                        style = MaterialTheme.typography.labelSmall,
                        color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.6f)
                    )
                    ...
                    LazyVerticalGrid(
                        ...
                        items(coverChoices.size) { i ->
                            val choice = coverChoices[i]
                            val img = remember(choice.releaseGroupId) {
                                try {
                                    android.graphics.BitmapFactory.decodeByteArray(
                                        choice.bytes, 0, choice.bytes.size
                                    )?.asImageBitmap()
                                } catch (_: Exception) {
                                    null
                                }
                            ...
                            Column(
                                Modifier
                                    .padding(4.dp)
                                    .clickable {
                                        vm.applyCoverChoice(track.uri, choice)
                                    }
                            ) {
                                if (img != null) {
                                    Image(
                                        bitmap = img,
                                        contentDescription = choice.album,
                                        modifier = Modifier
                                            .fillMaxWidth()
                                            .aspectRatio(1f),
            ...
            confirmButton = {
                TextButton(onClick = { vm.dismissCoverChoices() }) {
                    Text("Annuler")
                }
            }
        )
